...
@block("content")
  ...
  <div class="card card-inverse-light-with-black-text flatten-border">
    ...
    <div class="card-body">
      <form method="post" action="{{ route('admin.advertise.store') }}" id="create-advertise-form" enctype="multipart/form-data">
      {{ prevent_csrf() }}
        <div class="row">
          <div class="col">
...
<script>
  ...
  myDropzoneNewCollection.on("successmultiple", function(multiple,xhr) {
    ...
    if(response.status == 200) {
      ...
      $('#create-advertise-form').find('button[type="submit"]').html("Submit");
      $('#create-advertise-form').find('button[type="submit"]').prop('disabled', false);
